...
export const contactAdd = createAsyncThunk("contact/add", async (contact) => {
  try {
    let result = axios.post("http://localhost:5001/contact/add", contact);
    return result;
  } catch (error) {
    console.log(error);
  }
...
export const contactGet = createAsyncThunk("contact/get", async () => {
  try {
    let result = axios.get("http://localhost:5001/contact/");
    return result;
  } catch (error) {
    console.log(error);
  }
...
export const contactDelete = createAsyncThunk("contact/delete", async (id) => {
  try {
    let result = axios.delete(`http://localhost:5001/contact/${id}`);
    return result;
  } catch (error) {
    console.log(error);
  }
...
export const    contactUpdate = createAsyncThunk(
  ...
  async ({ id, contact }) => {
    try {
      let response = await axios.put(`http://localhost:5001/contact/${id}`, contact);
      return response;
    } catch (error) {
      console.log(error);
    }
...
export const contactSlice = createSlice({
  ...
  extraReducers: {
    ...
    [contactGet.fulfilled]: (state, action) => {
      ...
      state.accessoires = action.payload.data?.contacts;
    },
